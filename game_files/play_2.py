...
ASSET_PATH = "super-complex-pips-game/assets/play/"
...
class DominoTile:
    ...
    x: float
    y: float
    rotation: int = 0  # 0=v, 1=h, 2=v flipped, 3=h flipped
    placed: bool = False
...
class GameView(arcade.View):
    ...
    def setup(self):
        ...
        # Create dominoes (start HORIZONTAL in dock, stacked vertically)
        for idx, (top, bottom) in enumerate(self.domino_data):
            dock_y = DOCK_Y - idx * (TILE_SIZE + DOCK_SPACING)  # 1-tile tall when horizontal
            domino = DominoTile(
                top=top,
                bottom=bottom,
                x=DOCK_X,
                y=dock_y,
                rotation=1,           # start horizontal in the dock
                dock_index=idx
            )
    ...
    def return_to_dock(self, domino):
        ...
        dock_y = DOCK_Y - idx * (TILE_SIZE + DOCK_SPACING)  # same stacking as setup
        domino.x, domino.y = DOCK_X, dock_y
        domino.placed = False
        domino.rotation = 1  # return HORIZONTAL in dock
